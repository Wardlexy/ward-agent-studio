...
interface CharacterProps {
  ...
  isTyping?: boolean
  /** Click handler for ops inspection */
...
const Character: React.FC<CharacterProps> = ({
  ...
  const renderPosition = Number.isFinite(agent.position.x) && Number.isFinite(agent.position.y)
    ? agent.position
    : agent.targetPosition
  const prevPosRef = useRef({ x: renderPosition.x, y: renderPosition.y })
  ...
  const dx = renderPosition.x - prevPosRef.current.x
  const dy = renderPosition.y - prevPosRef.current.y
  ...
  prevPosRef.current = { x: renderPosition.x, y: renderPosition.y }
  ...
  return (
    <div
      className={`character-wrapper state-${animState}${onClick ? ' ops-clickable' : ''}${selected ? ' ops-selected' : ''}`}
      style={{
        left: `${renderPosition.x}%`,
        top: `${renderPosition.y}%`,
        transform: 'translate(-50%, -100%)',
        zIndex: zIndex ?? Math.round(renderPosition.y),
      }}
      role={onClick ? 'button' : undefined}
      tabIndex={onClick ? 0 : undefined}
      aria-label={onClick ? `Inspect ${agent.name}` : undefined}
    ...
    >
      {opsStatus && (
        <div className={`char-ops-badge status-${opsStatus}`}>
          <span>{opsLabel ?? opsStatus}</span>
          <div className="char-ops-progress">
            <i style={{ width: `${Math.max(0, Math.min(100, opsProgress))}%` }} />
          </div>
        </div>
      )}
