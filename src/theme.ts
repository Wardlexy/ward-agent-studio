...
const DEFAULT_SPAWN = [
  'clocked in at Orbit Lab', 'ready to build something useful', 'opening the idea board',
  'coffee first, then experiments', "let's make a small win", 'checking today\'s mission',
]
const DEFAULT_WORK = [
  'shaping the next step', 'turning idea into a task', 'in focus mode', 'making progress',
  'checking references', 'cleaning up the plan', 'testing if this actually helps',
]
const DEFAULT_DONE = [
  'mission complete', 'small win shipped', 'draft ready', 'done and saved',
  'looks useful', 'next step unlocked', 'ready for review',
]
const DEFAULT_COFFEE = ['brb, coffee', 'need caffeine', 'refueling for ideas', 'coffee run']
const DEFAULT_WATER = [
  'stay hydrated', 'h2o break', 'water run', 'refilling bottle',
  'hydration check', 'quick reset break',
]
